...
function formatNumber(num: any, formatted: boolean): FormatNumberResult {
  try {
    const parsedNum = Number(num);
    const safeNum = isNaN(parsedNum) ? 0 : Math.max(0, Math.floor(parsedNum));
    if (formatted) {
      if (safeNum < 1000) {
        return { number: [String(safeNum)], unit: '' };
      }
      const units = ['k', 'M', 'B', 'T'];
      let unitIndex = 0;
      let n = safeNum;
      ...
      const finalNumber = String(Math.floor(n));
      return { number: [finalNumber], unit: units[unitIndex - 1] ?? '' };
    } else {
      const formattedStr = String(safeNum).replace(/\B(?=(\d{3})+(?!\d))/g, ',');
      return { number: formattedStr.split(','), unit: '' };
    }
  } catch (e) {
...
function GitHubStarsButton({
  ...
  useEffect(() => {
    ...
    getGithubStars()
      .then((count) => {
        if (isMounted) setStars(count || 0);
      })
      .catch((err) => {
        console.error('Failed to get stars:', err);
      });
  ...
  const renderNumberSegments = (
    ...
    >
      {(segments || ['0']).map((segment, index) => (
        <Fragment key={index}>
          {Array.from(segment || '0').map((digit, digitIndex) => (
